...
function safeList(json,fallback=[]){try{const v=JSON.parse(json||'null');return Array.isArray(v)?v:fallback;}catch{return fallback;}}
...
routes['/workers/:id']=async(id)=>{const w=await api(`/workers/${id}`);let h=`<div class="card" style="text-align:center"><div class="avatar" style="margin:0 auto 8px;width:72px;height:72px;font-size:28px">${esc(w.name).charAt(0)}</div><div style="font-weight:800;font-size:18px">${esc(w.name)}</div><div style="font-size:13px;color:var(--muted)">⭐${w.profile?.rating_avg||'Chưa có'}·${w.profile?.jobs_done||0}việc·${w.profile?.years_exp||0}năm KN</div></div>`;if(w.profile){h+=`<div class="card"><div style="font-weight:700;margin-bottom:6px">Giới thiệu</div><div style="font-size:13px;color:var(--muted)">${esc(w.profile.bio||'Chưa có mô tả')}</div><div style="margin-top:10px"><span style="font-size:12px;color:var(--muted)">Kỹ năng:</span> ${(JSON.parse(w.profile.skills||'[]')).map(s=>SKILLS[s]||s).join(', ')}</div><div style="margin-top:6px;font-size:13px"><span style="color:var(--muted)">Khu vực:</span> ${(JSON.parse(w.profile.districts||'[]')).map(d=>DISTRICTS[d]||d).join(', ')}</div></div>`;}if(w.reviews?.length){h+=`<h3 style="margin:16px 0 10px">Đánh giá</h3>`;w.reviews.forEach(r=>h+=`<div class="card"><div style="font-weight:700">${r.rating}⭐</div><div style="font-size:13px;color:var(--muted)">${esc(r.comment||'')}</div><div style="font-size:11px;color:var(--muted);margin-top:4px">${fmtDate(r.created_at)}</div></div>`);}return shell(h,'Thợ');};
